...
if __name__ == '__main__':
    ...
    # 関数yの図示 x軸 x, y軸 y
    plt.plot(x1, y1, label='Runge-Kutta 4th Order (y)')
    plt.plot(x2, y2, label='Runge-Kutta 2nd Order (y)')
    plt.xlabel("x")
    plt.ylabel("y")
    plt.legend()
    plt.show()
    ...
    # 関数y'の図示 x軸 y, y軸 y'
    plt.plot(y1, dy_dx1, label="Runge-Kutta 4th Order (dy/dx)")
    plt.plot(y2, dy_dx2, label="Runge-Kutta 2nd Order (dy/dx)")
    plt.xlabel("y")
    plt.ylabel("y'")
    plt.legend()
    plt.show()
